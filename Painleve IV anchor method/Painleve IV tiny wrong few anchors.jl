# finite_n_piv_locked_perturbed_sigma_few_anchors.jl
#
# Finite-n GUE largest-eigenvalue CDF:
#   - Fredholm side via Hermite kernel + Nyström on (s, s+L(n))
#   - Painlevé side via the σ-form with a small perturbation +0.01:
#
#       (σ'')^2 = 4 (s σ' - σ)^2 - 4 (σ')^2 (σ' + 2n + 0.01)
#
# This file uses FEWER ANCHORS to see how much the locked method
# relies on the Fredholm "re-locking", and whether the perturbed
# σ-form can be distinguished from the correct one.
...
# ---------------------------
# Perturbed σ-form with +2n+0.01 and stepping with branch tracking
#
#   (σ'')^2 = 4 (s σ' - σ)^2 - 4 (σ')^2 (σ' + 2n + 0.01)
# ---------------------------
@inline function piv_rhs(s::Float64, σp::Float64, σ::Float64, n::Int)
    return 4.0*(s*σp - σ)^2 - 4.0*(σp^2)*(σp + 2.0*n + 0.01)
end
...
# ---------------------------
# One full run (PERTURBED σ-form, FEWER ANCHORS)
# ---------------------------
function finite_n_piv_locked_few_anchors(; n::Int=20, window_half::Float64=3.0,
                                          NquadFD::Int=260, npts::Int=1201, nanchors::Int=41)

    s_edge = sqrt(2.0*n)
    smin, smax = s_edge - window_half, s_edge + window_half
    @info "PERTURBED σ-form (+2n+0.01), FEWER ANCHORS: n=$n  edge≈$s_edge; domain [$smin,$smax]; Nquad=$NquadFD; anchors=$nanchors"
    ...
    plt1 = plot(sgrid, F_fd, lw=2, label="Finite-n Fredholm (Hermite kernel)",
                xlabel="s", ylabel="CDF",
                title="Finite-n GUE CDF (PERTURBED σ-form, FEWER ANCHORS, n=$(n))")
    plot!(plt1, sgrid, F_piv, lw=2, ls=:dash, label="σ-form (+2n+0.01) locked")
    savefig(plt1, "finite_n_piv_perturbed_few_anchors_vs_fd_n$(n).png"); display(plt1)

    plt2 = plot(sgrid, abs.(F_fd .- F_piv), lw=2, label="|Δ|",
                xlabel="s", ylabel="absolute error",
                title="Abs diff: PERTURBED σ-form, FEWER ANCHORS vs Fredholm (n=$(n))")
    savefig(plt2, "finite_n_piv_perturbed_few_anchors_absdiff_n$(n).png"); display(plt2)

    @info "PERTURBED σ-form (+2n+0.01), FEWER ANCHORS: n=$n: max |Δ| = $(maximum(abs.(F_fd .- F_piv)))"
    return (sgrid=sgrid, F_fd=F_fd, F_piv=F_piv, plt1=plt1, plt2=plt2)
end

# ---------------------------
# Run the suite with reduced anchors (PERTURBED σ-form)
# ---------------------------
function run_suite()
    configs = [
        # fewer anchors than the dense-anchors experiments
        (n=5,    N=220, anchors=21, window=3.5),
        (n=10,   N=240, anchors=31, window=3.5),
        (n=20,   N=260, anchors=41, window=3.2),
        (n=100,  N=280, anchors=51, window=3.0),
        (n=500,  N=300, anchors=61, window=3.0),
    ]
    for c in configs
        finite_n_piv_locked_few_anchors(n=c.n, NquadFD=c.N,
                                        nanchors=c.anchors, window_half=c.window)
    end
    @info "PERTURBED σ-form (+2n+0.01), FEWER ANCHORS: saved all figures in $(pwd())"
end
