# finite_n_piv_locked_perturbed_sigma.jl
#
#   (σ'')^2 = 4 (s σ' - σ)^2 - 4 (σ')^2 (σ' + 2n + 0.01)
#
# Same method as the “correct σ-form” code, but with a small +0.01
# perturbation in the parameter, and with n = 500 removed from the suite.
...
# ---------------------------
# Perturbed PIV σ-form and stepping with branch tracking
#
#   (σ'')^2 = 4 (s σ' - σ)^2 - 4 (σ')^2 (σ' + 2n + 0.01)
# ---------------------------
@inline function piv_rhs(s::Float64, σp::Float64, σ::Float64, n::Int)
    return 4.0*(s*σp - σ)^2 - 4.0*(σp^2)*(σp + 2.0*n + 0.1)
end
...
# ---------------------------
# One full run (PERTURBED σ-form, robust Nyström)
# ---------------------------
function finite_n_piv_locked_perturbed(; n::Int=20, window_half::Float64=3.0,
                                       NquadFD::Int=260, npts::Int=1201, nanchors::Int=81)

    s_edge = sqrt(2.0*n)
    smin, smax = s_edge - window_half, s_edge + window_half
    @info "PERTURBED σ-form (+0.01): n=$n  edge≈$s_edge; domain [$smin,$smax]; Nquad=$NquadFD; anchors=$nanchors"
    ...
    plt1 = plot(sgrid, F_fd, lw=2, label="Finite-n Fredholm (Hermite kernel)",
                xlabel="s", ylabel="CDF",
                title="Finite-n GUE CDF (PERTURBED σ-form, n=$(n))")
    plot!(plt1, sgrid, F_piv, lw=2, ls=:dash, label="PIV σ-form (+0.01, locked)")
    savefig(plt1, "finite_n_piv_perturbed_vs_fd_n$(n).png"); display(plt1)

    plt2 = plot(sgrid, abs.(F_fd .- F_piv), lw=2, label="|Δ|",
                xlabel="s", ylabel="absolute error",
                title="Abs diff: perturbed PIV vs Fredholm (n=$(n))")
    savefig(plt2, "finite_n_piv_perturbed_absdiff_n$(n).png"); display(plt2)

    @info "PERTURBED σ-form (+0.01): n=$n: max |Δ| = $(maximum(abs.(F_fd .- F_piv)))"
    return (sgrid=sgrid, F_fd=F_fd, F_piv=F_piv, plt1=plt1, plt2=plt2)
end

# ---------------------------
# Run the suite (PERTURBED σ-form, n = 5,10,20,100)
# ---------------------------
function run_suite()
    configs = [
    ...
    for c in configs
        finite_n_piv_locked_perturbed(n=c.n, NquadFD=c.N,
                                      nanchors=c.anchors, window_half=c.window)
    end
    @info "PERTURBED σ-form (+0.01): saved all figures in $(pwd())"
end
