# finite_n_piv_locked_wrong1_sigma.jl
# Finite-n GUE largest-eigenvalue CDF:
#   - Fredholm side via Hermite kernel + Nyström on (s, s+L(n))
#   - Painlevé side via the wrong σ-form PIV for finite-n GUE, locked to Fredholm.
#
# wrong σ-form (Jimbo–Miwa–Okamoto / Tracy–Widom style for GUE):
#   (σ'')^2 = 4 (s σ' - σ)^2 - 4 (σ' - n)(σ' - n - 1) σ'
...
# ---------------------------
# Correct σ-form PIV and stepping with branch tracking
#
# (σ'')^2 = 4 (s σ' - σ)^2 - 4 (σ' - n)(σ' - n - 1) σ'
# ---------------------------
@inline function piv_rhs(s::Float64, σp::Float64, σ::Float64, n::Int)
    return 4.0*(s*σp - σ)^2 - 4.0*(σp - n)*(σp - n - 1.0)*σp
end
...
function finite_n_piv_locked_correct(; n::Int=20, window_half::Float64=3.0,
    ...
    plt1 = plot(sgrid, F_fd, lw=2, label="Finite-n Fredholm (Hermite kernel)",
                xlabel="s", ylabel="CDF",
                title="Finite-n GUE CDF (correct σ-form PIV, n=$(n))")
